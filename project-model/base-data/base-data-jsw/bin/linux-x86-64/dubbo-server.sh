...
# Optimized for use with version 3.5.32-st of the Wrapper.
...
launchinternal() {
    ...
    then 
        ...
        COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" wrapper.daemonize=TRUE $APPNAMEPROP $ANCHORPROP $IGNOREPROP $STATUSPROP $COMMANDPROP $LOCKPROP wrapper.script.version=3.5.32 $ADDITIONAL_PARA"
        eval $COMMAND_LINE
    else
...
console() {
    ...
    then
        ...
        # The string passed to eval must handles spaces in paths correctly.
        COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" $APPNAMEPROP $ANCHORPROP $STATUSPROP $COMMANDPROP $LOCKPROP wrapper.script.version=3.5.32 $ADDITIONAL_PARA"
        eval $COMMAND_LINE
        COMMAND_EXIT_CODE=$?
...
upstartDetection() {
    ...
    result='initctl version | grep upstart'
    # if the word "restart" is in the result, then we assume upstart is installed.
    if [ $? -eq 0 ] ; then
        return 0
    fi
    return 1
...
startdaemon() {
    if [ "$DIST_OS" = "macosx" -a -f "/Library/LaunchDaemons/${APP_PLIST}" ] ; then
        ...
        # In case the user set SERVICE_MANAGEMENT_TOOL to something different than 'auto' or 'initd',
        # then we show a warning saying that because he didn't run "install", it will not use 
        # the service management tool to start the service.
        if [ "$SERVICE_MANAGEMENT_TOOL" != "auto" -a "$SERVICE_MANAGEMENT_TOOL" != "initd" ] ; then
            eval echo `gettext 'Script configured to use a service management tool $SERVICE_MANAGEMENT_TOOL, but the service was not installed.'`
        fi
...
# Resolve the service management tool for 3 platforms: linux, solairs, aix.
